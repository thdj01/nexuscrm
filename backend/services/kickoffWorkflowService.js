...
async function sendKickoffNotifications({ inquiry, workflow, attendees, scheduledBy }) {
  ...
  await dispatchNotificationsToUsers({
    ...
    message: `Kick-off Meeting for inquiry ${inquiry.inquiryId || '-'} was scheduled by ${kickoffScheduledByName} for ${formatDateTime(workflow.scheduledAt)}.`,
...
async function createProjectFromWorkflow(workflow, convertedBy = {}) {
  ...
  const projectData = {
    ...
    orderDate: new Date(),
    orderEndDate: inquiry.orderEndDate || undefined,
