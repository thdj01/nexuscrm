...
const InquiriesPage = () => {
  ...
  const columns = [
    ...
    {
      key: '_id', label: 'Actions', width: '180px',
      render: (_, row) => {
        ...
        return (
          ...
          {isOrderWonStatus(row.status) && !row.convertedToProject && row.kickoffMeeting?.status !== 'Scheduled' && (
            <button
              type="button"
              onClick={(e) => {
                e.stopPropagation();
          ...
          {isOrderWonStatus(row.status) && !row.convertedToProject && isKickoffScheduledOrReady(row) && (
            <button
              type="button"
              onClick={(e) => {
                e.stopPropagation();
  ...
  return (
    <div className="fade-in min-w-0 space-y-4">
      ...
      <Modal isOpen={kickoffModal} onClose={closeKickoffModal} title={kickoffMode === 'view' ? 'Kick-off Meeting' : kickoffMode === 'edit' ? 'Edit Kick-off Meeting' : 'Schedule Kick-off Meeting'} size="md">
        <div className="space-y-5">
          <div>
