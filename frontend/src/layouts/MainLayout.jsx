...
const MainLayout = () => {
  ...
  const isInquiryFormPage = location.pathname === '/inquiries/new'
    || /^\/inquiries\/[^/]+(?:\/edit)?$/.test(location.pathname);
  const isCustomerFormPage = location.pathname.startsWith('/customers/') && location.pathname !== '/customers';
  const isTicketFormPage = location.pathname === '/tickets/new'
    || /^\/tickets\/[^/]+(?:\/edit)?$/.test(location.pathname);
  const isFlushFormPage = isProjectDetailPage || isInquiryFormPage || isCustomerFormPage || isTicketFormPage;
  ...
  return (
    <div className="flex h-screen overflow-hidden bg-gray-50">
      ...
      */}
      ...
      >
        <Topbar onMenuClick={() => setMobileOpen(true)} />
        <main className={`min-w-0 flex-1 overflow-y-auto overflow-x-hidden px-3 sm:px-5 xl:px-6 ${isFlushFormPage ? 'pb-0 pt-0 sm:pb-0 sm:pt-0 xl:pb-0 xl:pt-0' : 'pb-3 pt-3 sm:pb-5 sm:pt-5 xl:pb-6 xl:pt-6'}`}>
          <Outlet />
        </main>
      </div>
    </div>
